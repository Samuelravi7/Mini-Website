...
<head>
    ...
    <title>ScanMaster : Search Product</title>
    <!-- Bootstrap -->
            ...
            <form action="<?php echo $_SERVER['REQUEST_URI']; ?>" method="post">
                <div class="col-sm-offset-2 col-sm-8">
                    <div class="form-group">
                        <label for="inputKeyword">Keywords (Brands, Price, Size)</label>
                        <input type="text" class="form-control" id="inputKeyword" name="search"
                               placeholder="Eg. Canon Deskjet 1">
                    </div>
                </div>
            ...
            <table class="table table-bordered">
                ...
                if (isset($_POST['search'])) {
                    ...
                    if (count($keywords) == 3) {
                        ...
                        $stmt = $db->prepare("SELECT * FROM tbl_products_a194594_pt2 WHERE fld_product_brand LIKE :brand AND FLD_PRICE LIKE :price AND fld_product_size LIKE :size ORDER BY fld_product_num ASC");
                        $stmt->bindParam(":brand", $brand);
                        $stmt->bindParam(":price", $price);
                        $stmt->bindParam(":socket", $size);

                        $stmt->execute();
                        $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
                    } elseif (count($keywords) == 1) {
                        $search = "%{$keywords[0]}%";
                        $stmt = $db->prepare("SELECT * FROM `tbl_products_a194594_pt2` WHERE fld_product_name LIKE :param OR fld_product_price LIKE :param OR fld_product_brand LIKE :param ORDER BY fld_product_num ASC");
                    ...
                    $stmt = $db->query("SELECT * FROM tbl_products_a194594_pt2 ORDER BY fld_product_quantity ASC LIMIT 0,10");
                    $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
                }
                ...
                if (count($result) > 0) {
                    foreach ($result as $readrow) {
                        ...
                        ?>
                        <tr style="color: #AAA;">
                            <td><?php echo sprintf("MB%03d", $readrow['fld_product_num']); ?></td>
